# Stress harness for tip 298060b (PR #4). Physics/math first, then CPU layout.
...
@testset "fixed points E=0" begin
    ...
    # inverted is a 1st-order fixed point but NOT 2nd-order: +2ig Sz + ig SmSp
    # cancels only when SmSp = N and Sz = −N/2 (ground).
    uinv = SCM.build_u0_2nd_order(Mb, Nj, Float64, :inverted)
    duinv = zeros(ComplexF64, length(uinv))
    SCM.rhs2!(duinv, uinv, p2, 0.0)
    @test maximum(abs, duinv) > 1.0
end
...
# ---------------------------------------------------------------------------
# CPU efficiency — measured, targets marked broken for Demiurge
# ---------------------------------------------------------------------------
@testset "CPU efficiency: hot-path allocs / no threading / layout" begin
    src = read(joinpath(@__DIR__, "..", "src", "SpinCavityMonolith.jl"), String)
    @test !occursin("Threads.@", src)          # no CPU multicore in the monolith
    @test occursin("sumP = zeros(T, M)", src)  # scratch re-allocated every RHS
    @test occursin("zero(small), [zero(L) for L in larges]", src)

    Mb = 8
    Nj = fill(4.0, Mb)
    delta_b = 1e5 .* [0.1, -0.2, 0.3, -0.4, 0.15, -0.05, 0.25, -0.35]
    g_b = fill(2π * 100, Mb)
    u = SCM.build_u0_2nd_order(Mb, Nj, Float64, :equator)
    u[1] = 0.01 + 0.002im
    s, L, c, o = SCM.dense_to_shards(u, Mb, 1)
    ds = zero(s); dL = [zero(L[1])]
    args = (ds, dL, s, L, c, o, 0.0, 2π * 1e6, 2π * 2e5, delta_b, g_b, Mb, 0.1 + 0.05im)
    SCM.rhs2_sharded!(args...)
    n_rhs = @allocated SCM.rhs2_sharded!(args...)
    println("[stress] rhs2_sharded! bytes/RHS = ", n_rhs, "  TARGET=0")
    u1 = SCM.build_u0_1st_order(Mb, Nj, Float64, :weak)
    du1 = zeros(ComplexF64, length(u1))
    p1 = (0.0, 2π * 1e6, 2π * 2e5, delta_b, g_b, Mb, t -> 0.1 + 0.0im)
    SCM.rhs1!(du1, u1, p1, 0.0)
    n1 = @allocated SCM.rhs1!(du1, u1, p1, 0.0)
    println("[stress] rhs1! bytes/RHS = ", n1, "  TARGET=0")
    @test n1 == 0
    # Demiurge: rhs2_sharded! currently mallocs 6 length-M scratch vectors every call.
    @test_broken n_rhs == 0
    ...
    println("[stress] bspline E(t) bytes/eval = ", nE, "  TARGET=0")
    @test_broken nE == 0

    m = match(r"function integrate_order2_sharded![\s\S]+?function solve_2nd_order", src)
    body = m === nothing ? "" : m.match
    println("[stress] order2 integrator body mentions ck45? ", occursin("ck45", lowercase(body)))
    @test_broken occursin("ck45", lowercase(body))
end

println("SpinCavityMonolith stress harness finished.")
