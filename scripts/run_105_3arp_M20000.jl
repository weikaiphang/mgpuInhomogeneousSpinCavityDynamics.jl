# ============================================================
# 3-ARP composite pi-pulse on the run_105 setup (M = 20000)
#
# Base config: data/sweep_1st_order/run_105.jld2 -- the setup in that
# sweep with BOTH a lorentzian frequency inhomogeneity (FWHM = 2pi*1e6)
# AND a gaussian coupling inhomogeneity (std = 2pi*0.511 Hz), already at
# M_delta * M_g = 1000 * 20 = 20000.
#
# This script:
#   1. loads run_105's SIM_SETTING / SYSTEM_CONFIG verbatim (only
#      saved_file_name is repointed to the new output),
#   2. replaces PULSE_CONFIG with a 3-segment (+k, +k/2, +k) ARP
#      composite pi-pulse -- the exact analytic form
#      generate_2n1_arp_pi_pulse(d; n_pairs=1) produces (built inline
#      here so its internal CPU metrics solves are skipped; the GPU run
#      below is the authoritative measurement),
#   3. writes a config-only stub .jld2,
#   4. runs the 1st-order GPU simulation and rewrites the same .jld2
...
# ---- 2. 3-ARP composite pi-pulse (generate_2n1_arp_pi_pulse, n_pairs=1) --
# Universal ratios: durations 1:2:1, chirp rates +k,+k/2,+k, amplitudes
# 1 : 1/sqrt(2) : 1. Absolute scales from d (see composite_arp_pulses.jl).
bw       = 5.0 * d.FWHM                                        # bandwidth_fwhm_mult = 5.0
Tb       = 0.6 * (d.timespan[2] - d.timespan[1])              # default T_budget
dur_odd  = Tb / 4
dur_even = 2 * dur_odd
amp_scale    = d.kappa_t / (4 * d.g_mean * d.sqrt_kappa_e)
Omega_target = pi * d.FWHM                                     # default Omega_max target
...
# ---- 4. run the GPU simulation, rewrite the file with full results ------
...
# augment the just-saved file with the ARP parameters + base-run tag
